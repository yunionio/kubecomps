...
func TestGenerateCommandOptions(t *testing.T) {
	tests := []struct {
	...
	}{
		{
			...
			err:                    errors.Error("(ansible::GenerateCommandOptions): AnsiblePlaybookOptions is nil"),
			options:                nil,
		},
		{
		...
		{
			desc: "Testing AnsiblePlaybookOptions with extra vars file",
			ansiblePlaybookOptions: &AnsiblePlaybookOptions{
				ExtraVarsFile: "/tmp/vars.json",
			},
			err:     nil,
			options: []string{"--extra-vars", "@/tmp/vars.json"},
		},
		{
			desc: "Testing AnsiblePlaybookOptions with extra vars and file at same time",
			ansiblePlaybookOptions: &AnsiblePlaybookOptions{
				ExtraVarsFile: "/tmp/vars.json",
				ExtraVars: map[string]interface{}{
					"extra": "var",
			...
			err:     errors.Errorf("(ansible::GenerateCommandOptions): use extraVars and extraVarsFile at same time"),
			options: nil,
		},
	}
...
func TestAddExtraVar(t *testing.T) {
	tests := []struct {
		...
		},
		{
			...
			err:           errors.Errorf("(ansible::AddExtraVar) ExtraVar '%s' already exist", "extra"),
			extraVarName:  "extra",
			extraVarValue: "var",
			res:           nil,
		},
...
func TestRun(t *testing.T) {
	...
	tests := []struct {
	...
	}{
		{
			...
			res:                "",
			err:                errors.Error("(ansible:Run) AnsiblePlaybookCmd is nil"),
		},
		{
